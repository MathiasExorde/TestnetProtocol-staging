...
library AttributeStore2 {
    ...
    function getAttribute(ComplianceData  storage self, bytes32 _UUID, string memory _attrName)
    ...
    function setAttribute(ComplianceData  storage self, bytes32 _UUID, string memory _attrName, uint256 _attrVal)
    public {
        bytes32 key = keccak256(abi.encodePacked(_UUID, _attrName));
...
library DLL2 {
  ...
  */
  function insert(ComplianceData storage self, uint256 _prev, uint256 _curr, uint256 _next) public {
...
contract DataCompliance is Ownable, RandomAllocator {
    ...
    //         - Frozen Compliance Stakes: 0.3%
    //         - Frozen Compliance-Checking Stakes: 0.2%
    //         - Frozen Compliance Stakes: 0.2%
    //         - Frozen Compliance-Checking Stakes: 0.1%
    ...
    event _ComplianceCheckRevealed(uint256 indexed DataID, uint256 numTokens, uint256 votesFor, uint256 votesAgainst, uint256 indexed choice, address indexed voter);
    ...
    */
    ...
    function AllocateWork() public  {
        ...
        uint256 selected_k = Math.max( Math.min(availableWorkers.length, Parameters.get_COMPLIANCE_MAX_CONSENSUS_WORKER_COUNT()), Parameters.get_COMPLIANCE_MIN_CONSENSUS_WORKER_COUNT()); // pick at most CONSENSUS_WORKER_SIZE workers, minimum 1.
    ...
    */
    function revealComplianceCheck(uint256 _DataBatchId, uint256 _voteOption, string memory _clearIPFSHash, uint256 _salt) topUpSFuel public {
        ...
        if (_voteOption == 1) {// apply numTokens to appropriate ComplianceData choice
        ...
        AllTxsCounter += 1;
        emit _ComplianceCheckRevealed(_DataBatchId, numTokens, DataBatch[_DataBatchId].votesFor, DataBatch[_DataBatchId].votesAgainst, _voteOption, msg.sender);
    }
