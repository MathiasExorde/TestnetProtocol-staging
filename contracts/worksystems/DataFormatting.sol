...
contract DataFormatting is Ownable, RandomAllocator {
    ...
    function RegisterWorker() public topUpSFuel {
        ...
        if (FormatStakedTokenBalance[msg.sender] < _numTokens){ // if not enough tokens allocated to this worksystem: check if master has some, or try to allocate
            if (_senderMaster != address(0)){
                ...
                if (FormatStakedTokenBalance[_senderMaster] < _numTokens){
                    uint256 remainder = _numTokens.sub(FormatStakedTokenBalance[_senderMaster]);
    ...
    */
    function requestAllocatedStake(uint256 _numTokens, address _user) internal {
        ...
        FormatStakedTokenBalance[_user] += _numTokens;
